...
public interface IChannel : IAsyncDisposable, IDisposable
{
     ...
     Task<QueueDeclareOk> QueueDeclareAsync(
         ...
         bool exclusive,  // [ 是否排他 ] true：只允许当前 Connection 访问 ( 连接断开后队列自动删除 )
         bool autoDelete, // [ 是否自动删除 ] true：最后一个消费者断开连接后自动删除队列
         
         # [ 高级配置 ]
         # - { "x-max-priority", int } - 消息优先级 ( 0 - 255 )
...
public interface IChannel : IAsyncDisposable, IDisposable
{
     ...
     ValueTask BasicPublishAsync<TProperties>(
         string exchange,   // 1.直连交换机 Dirct.Exchange : ( exchange = "" + routingKey = "队列名称" ) 
         string routingKey,
         
         # [ 消息确认 ]
         # - true: 如果交换机存在 [ 但消息无法根据 routingKey 路由到任何队列 ] 
         #         ( MQ 将消息退回给生产者 : 触发事件 IChannel.BasicReturnAsync - 如果事件未订阅则抛出异常 )
...
#### **[ ℹ️ 消息载体 ] BasicProperties**

```csharp
public sealed class BasicProperties : IBasicProperties, IAmqpHeader
{
    
     # [ 消息头部 ] 在消息传递过程中携带额外的元数据 ( 消费者收到消息后可通过 Headers 读取这些扩展信息 )
     ...
     # [ 过期时间 ] 字符串: 单位毫秒 ( 优先级高 )
     # [ 注意事项：超时未被消费的消息 - 会被队列丢弃或转发到 - 死信队列 ]
...
public interface IAsyncBasicConsumer
{
     ...
     Task HandleBasicDeliverAsync(
          ...
          IReadOnlyBasicProperties properties, // [ 消息载体 ] BasicProperties ( 只读 )
          ReadOnlyMemory<byte> body,           // [ 消息内容 ] 
