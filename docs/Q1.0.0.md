...
>
>- <span style='color:Blue'>[ 故障情况`A`]</span> 消息未成功发送到`MQ`<span style='color:Blue'>( 生产者`Producer`)</span>
>
...
>```csharp
># [ 发送消息 ]
>await channel.BasicPublishAsync(exchange: "E2",
>                                 routingKey: "Email1",
>                                
># [ 消息确认模式 ]
># - mandatory = true: 如果交换机存在, 当消息无法根据 routingKey 路由到任何队列时 
>#                 ( RabbitMQ 将消息退回给生产者 : 触发事件 IChannel.BasicReturnAsync - 如果事件未订阅则抛出异常 )
